...
5.  What data types are contained in the variables? (Reminder: <https://ds4owd-001.github.io/website/slides/lec-02-visualisation.html#/types-of-variables>)
    ...
    Data types contained are factors (for categorical data), double (for decimals) and integers (for whole numbers)
...
```{r}
penguins |> 
  mutate(flipper_m=flipper_length_mm/100)
```
...
```{r}
penguins |> 
  mutate(boddy_mass_kg=body_mass_g/100)
