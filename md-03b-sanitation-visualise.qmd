...
```{r}
sanitation_malawi_2000_2020<-sanitation |>
  rename(country=name) |> 
  filter(country=="Malawi", year %in% c(2000, 2020),varname_short=="san_sm")
...
ggplot(sanitation_malawi_2000_2020,
       aes(x=residence,
           y=percent,
           fill="varname_long"))+
  geom_col()+
...
1.  Look at the plot that you created. What do you notice about the order of the bars / order of the legend?
    ...
    -The order of the bars is arranged alphabetically (in descending) order of the labels

2.  What would you want to change?

    -I would want to change the order of the bars to be shown in descending order of the percent value
...
2.  What do you observe when you look at the code and plot?
    ...
    -The plot is for only 2020

    -Different plots have been faceted into grids where the vertical variables are the residence (rural, etc) and the horizontal variables are the types of sanitation
